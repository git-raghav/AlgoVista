function processGrid(rowCount, columnCount, width, height, boxSize) {
	...
	states.Context.ActiveGrid.onStartEndSet = function () {
		if (states.Context.ActiveGrid.startNode != null && states.Context.ActiveGrid.endNode != null) {
	...
	states.Context.ActiveGrid.onRunnerStop = function () {
		...
		states.runnerDuration.text(`${states.Context.ActiveGrid.runner.duration} ms`);
...
function init() {
	...
	states.algoSelection.click(function (event) {
		if (!states.Context.ActiveGrid.runner.running || states.Context.ActiveGrid.runner.speed == null) {
			const algo = event.target.dataset["algo"];
			const extraData = event.target.dataset;
			if (states.Context.ActiveGrid.runner && !states.Context.ActiveGrid.runner.finish) {
				states.Context.ActiveGrid.runner.stop();
			}
			states.Context.ActiveGrid.setRunner(algo, extraData);
			resetGrid();
			if (states.Context.ActiveGrid.startNode && states.Context.ActiveGrid.endNode) {
