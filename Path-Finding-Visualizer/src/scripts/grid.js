class Box extends GraphNode {
	...
	resetTraversed() {
		if (this.nodeType == states.BOX_TYPES.TRAVERSED_NODE || this.nodeType == states.BOX_TYPES.PATH_NODE) {
			this.setAsClear();
		}
	}
...
class Grid {
	...
	fixGrid() {
		for (let r = 0; r < this.graph.rowCount; r++) {
			for (let c = 0; c < this.graph.columnCount; c++) {
				if (this.boxes[r][c].nodeType == states.BOX_TYPES.BLOCK || this.boxes[r][c].nodeType == states.BOX_TYPES.ERROR_NODE) {
					this.setBlock(r, c);
				}
			}
		}
