...
class sortAlgorithms {
	constructor(time) {
        ...
        this.maxValue = Math.max(...Array.from(this.list).map(bar => Number(bar.getAttribute("value")))); // Calculate once
	}
	...
	Merge = async (start, mid, end) => {
		...
		for (let c = start, point = 0; c <= end && point < newList.length; ++c, ++point) {
            ...
            this.list[c].style.height = `${(newList[point] / this.maxValue) * HEIGHT_SCALING_FACTOR}px`;
			// this.list[c].style.height = `${(newList[point] / 100) * HEIGHT_SCALING_FACTOR}px`;
			this.list[c].innerText = newList[point]; // Update displayed value
		}
		for (let c = start; c <= end; ++c) {
